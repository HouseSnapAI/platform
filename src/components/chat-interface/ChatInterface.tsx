...
const ChatInterface = ({ setInputValue, inputValue, chatHistory, handleClick, userInfo, loading, resetChat}: ChatInterfaceProps) => {
  ...
  return (
    <Box onClick={() => {if(!open) setOpenFunction(true)}} className={!open ? 'transition-all duration-200 ease-in-out absolute bg-[#181818] bottom-4 right-4 rounded-xl flex items-center justify-center w-[70px] h-[70px] drop-shadow-lg hover:cursor-pointer hover:drop-shadow-xl' : (expanded ? `transition-all duration-200 ease-in-out absolute bg-[#141414] bottom-4 right-4 rounded-lg flex flex-col items-center text-center justify-between w-[450px] h-[580px] drop-shadow-lg` : `transition-all duration-200 ease-in-out absolute bg-[#141414] bottom-4 right-4 rounded-lg flex flex-col items-center text-center justify-between w-[380px] h-[440px] drop-shadow-lg`)} sx={{border: `1px solid #222`}}>
        {
